...
router.get('/', authenticateToken, (req, res) => {
  try {
    ...
    res.json({
      ...
      kotPrinters: safeParse(settings.kot_printers) || [],
  ...
  try {
    ...
    db.prepare(`
      UPDATE settings SET
        ...
        kot_printers = COALESCE(?, kot_printers),
        default_kot_printer = COALESCE(?, default_kot_printer),
    ...
    `).run(
      ...
      kotPrinters ? JSON.stringify(kotPrinters) : null,
      defaultKotPrinter,
      billPrinter,
      printCopies,
      skipLinesBeforeCut,
